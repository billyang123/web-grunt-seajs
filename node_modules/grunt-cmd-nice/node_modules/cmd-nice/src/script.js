...
Script.prototype.execute = function(inputFile) {
    ...
    // Step 4: 得到依赖的模块，根据seajs.path&seajs.alias替换dependencies
    ...
    // Step 5: 递归查找依赖的依赖；依赖模块路径转换&合并
    var newDependencies = [];
    _.each(dependencies, function(dependency) {
        var isVars = /\{.*?\}/.test(dependency);
    ...
    var modifyOptions = {
        ...
        dependencies: _.map(newDependencies, function(dependency) {
            ...
            _.each(self.options.paths, function(pathname) {
                if (existsSync(path.join(pathname, dependency)) || existsSync(path.join(pathname, dependency) + ".js")) {
                    isLocalDependency = true;
                }
            });

            var isVars = /\{.*?\}/.test(dependency);
            var isVarsExisted = false;
            if (isVars) {
                _.each(self.options.paths, function(pathname) {
                    if (existsSync(path.join(pathname, path.dirname(dependency)))) {
                        isVarsExisted = true;
                    }
                });
            }
            ...
            } else if (_.isFunction(self.options.idRule) && isVarsExisted) {
                return self.options.idRule.call(self, dependency);
            } else {
                return dependency;
            }
        ...
        require: function(name) {
            ...
            var isLocalDependency = false;
            _.each(self.options.paths, function(pathname) {
                if (existsSync(path.join(pathname, newName)) ||
                    existsSync(path.join(pathname, newName) + ".js")) {
                    isLocalDependency = true;
                }
            });
            ...
            if (isVars) {
                _.each(self.options.paths, function(pathname) {
                    if (existsSync(path.join(pathname, path.dirname(newName)))) {
                        isVarsExisted = true;
                    }
                });
            }
...
Script.prototype.findDependencies = function(dependency, basePath) {
    ...
    realFilePath = path.normalize(realpathSync(realFilePath));
    ...
    var ast = null;
    var metaAst = null;
    if (self.options.useCache && _.has(self.astCache, realFilePath) && self.astCache[realFilePath].metaAst) {
 ...
 * 相对路径转成绝对路径
 * */
Script.prototype.getRealName = function(dependency, base, opt) {
    var self = this;
    var options = {
        isVars: false
    };
    ...
    if (dependency.indexOf("../") === 0 || dependency.indexOf("./") === 0) {
        ...
        if (!options.isVars) {
            // 默认查找js文件
            if (!existsSync(realName) && !/\.js$/.test(realName)) {
                realName += ".js";
            }
...
function realpathSync(fileName) {
    if (/\?.*/.test(fileName)) {
        fileName = fileName.replace(/\?.*/, '');
    }
